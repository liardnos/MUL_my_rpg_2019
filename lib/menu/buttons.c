...
void draw_buttons(win_t *win, button_t **buttons)
{
    ...
    while (*cpy) {
        if ((*cpy)->menu == win->menu) {
            ...
            if ((*cpy)->pressed) {
                sfSprite_setPosition(win->click->sprite, pos);
                sfRenderWindow_drawSprite(win->win, win->click->sprite, 0);
            }
        }
        cpy++;
    }
...
void reset_pressed(win_t *win, button_t **buttons)
{
    button_t **cpy = buttons;
    sfVector2i pos = sfMouse_getPosition((sfWindow *)win->win);
